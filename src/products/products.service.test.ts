...
import { createCategory } from '../../shared/test/helpers';
import { categoryData, productData } from '../../shared/test/samples';
...
describe('ProductsService', () => {
  ...
  before(async () => {
    ...
    category1 = createCategory(categoryData.categoryData1)
    category2 = createCategory(categoryData.categoryData2)

    product1 = productData.productData1
    product1 = productData.productData2
    product1 = productData.productData3
    product1 = productData.productData4
  ...
  it('should create a product with valid data', async () => {
    ...
    const result = await testingContainer.services.productsService.create(

      {...productData.productData1, categoryId: category1.id}

      );

    assert.strictEqual(result.name, product1.name);
    assert.strictEqual(result.description, product1.description);
    assert.strictEqual(result.unitsOnStock, product1.unitsOnStock);
    assert.strictEqual(result.price, product1.price);
    assert.strictEqual(result.category, product1.name);
  });
  ...
  it('should update a product with valid data', async () => {
    ...
    const result = await testingContainer.services.productsService.update(
      product.id,
      ProductPartialData,
    );
    ...
    assert.strictEqual(
      parseFloat(String(result.price)),
      ProductPartialData.price,
    );
  });
  ...
  it('should filter products with valid partial filter data', async () => {
    ...
    const res1: any =
      await testingContainer.services.productsService.getFilteredProducts(
        partialFilterData1,
      );
    const res2: any =
      await testingContainer.services.productsService.getFilteredProducts(
